...
# cairosvg has no font fallback: every glyph must exist in the face fontconfig
# resolves ("Helvetica" -> TeX Gyre Heros). We build the allowed set straight
# from the real cmap of both the regular and bold OTFs (a glyph must be in
# BOTH), so any missing arrow/tick/symbol raises at draw time instead of
# silently rendering as .notdef. Known gaps in this face include U+2194 (<->),
# U+21C5, U+2713 (check) -- draw those as SVG paths instead.
_FORBIDDEN_FALLBACK = {"⇅", "✓", "✔", "☑", "↔", "△", "▷",
                       "①", "②", "③", "④", "⑤", "⑥"}


def _load_covered():
    """Codepoints present in BOTH resolved faces, from their real cmaps."""
    try:
        from fontTools.ttLib import TTFont
        covered = None
        for ttf in (SANS_REG_TTF, SANS_BOLD_TTF):
            cmap = set(TTFont(ttf).getBestCmap().keys())
            covered = cmap if covered is None else (covered & cmap)
        # whitespace the layout relies on is always fine
        covered |= {ord(c) for c in " \n\t"}
        return covered
    except Exception:
        return None


_COVERED = _load_covered()


def assert_renderable(s, where=""):
    """Raise if `s` contains any character the diagram font cannot render."""
    if _COVERED is not None:
        bad = sorted({c for c in s if ord(c) not in _COVERED})
        if bad:
            raise ValueError(
                "glyph(s) %r (U+%s) missing from the diagram font (%s)"
                % (bad, "/".join("%04X" % ord(c) for c in bad), where))
    else:
        bad = sorted(set(s) & _FORBIDDEN_FALLBACK)
        if bad:
